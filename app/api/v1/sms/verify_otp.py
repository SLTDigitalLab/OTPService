...
def verify_sms_otp(tenent: Tenent, otp: str, mobile: str, client_secret: str) -> Success | Error:
    ...
    if result:
        ...
        if real_otp and real_otp == str(otp):
            ...
            elif result.get('expired_at') and  (datetime.timestamp(datetime.now()) - datetime.timestamp(result.get('expired_at'))) <= 0:
                return Error("OTP is expired.", 1000, 400)
            elif result.get('tries') and result.get('tries') >= tenent.sms_otp_max_tries:
                return Error("OTP is expired.", 1000, 400)
            else:
                x = db.exec("UPDATE sms_otp SET validated = %s, validated_at = %s WHERE id = %s",
                    (
                        True,
                        datetime.now(),
                        result.get('id'),
            ...
            db.exec("UPDATE sms_otp SET tries = %s WHERE id = %s",
                (
                    result.get('tries') + 1,
                    result.get('id'),
                )
    ...
    return Error("Invalid request, please check client secret and the mobile number is valid", 1000, 400)
