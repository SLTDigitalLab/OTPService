...
def verify_email_otp(
    tenent: Tenent, otp: str, mobile_number: str, client_secret: str
) -> Success | Error:
    ...
    if result:
        ...
        if real_otp and real_otp == str(otp):
            if result.get("validated"):
            ...
            elif result.get("expired_at") and (
                ...
                db.exec(
                    "UPDATE email_otp SET validated = %s, validated_at = %s WHERE id = %s",
                    (
                        True,
                        datetime.now(),
                        result.get("id"),
            ...
            db.exec(
                "UPDATE email_otp SET tries = %s WHERE id = %s",
                (
                    result.get("tries") + 1,
                    result.get("id"),
                ),
    ...
    return Error(
        "Invalid request, please check client secret and the email is valid", 1000, 400
    )
